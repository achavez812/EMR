...
<?php
	...
	$exam_categories = $map->getExamCategories();
...
<div class="container-fluid">
	...
	<div class="row row2">
		<div class="col-xs-12">
			...
			<p class="content_p consult_section">Exams</p>
		</div>
	</div>

<?php

	for($index = 0; $index < sizeof($exam_categories); $index++) {
		$exam_category = $exam_categories[$index];
?>
		<li class="list-group-item" onclick="functionClick(<?php echo $consult_id . ", " . $index; ?>);"> 
			<?php
			$has_normal_exam = $db->hasConsultNormalExamCategory($consult_id, $index);
			$has_abnormal_exam = $db->hasConsultAbnormalExamCategory($consult_id, $index);
			if($has_normal_exam) {
				$exam_category .= " (Normal)";
			} else if($has_abnormal_exam) {
				$exam_category .= " (Abnormal)";
			}
			echo $exam_category;
			 if($has_normal_exam || $has_abnormal_exam) { echo '<img class="consult_task_completed" src="../images/checkmark"/>'; } ?>
		</li>
<?php		
	}
...
?>
	...
	<div id="button_row" class="row input_row">
		<div id="button_col" class="col-xs-12">
			<button class="consult_button" type="button" onclick="goBackFunction(<?php echo $consult_id; ?>);">Back to Chief Complaints</button>
		</div>
	</div>
...
function goBackFunction(consult_id) {
	window.location.href = "chief_complaints.php?consult_id=" + consult_id;
}

function functionClick(consult_id, exam_category_index) {
	window.location.href = "exams2.php?consult_id=" + consult_id + "&category=" + exam_category_index;
}
