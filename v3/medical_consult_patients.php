...
<?php
	...
	if(isset($_GET['show_alert'])) {
		...
		if($show_alert == 2) {
			echo '<script>';
			echo 'alert("' . HOME_SET_MEDICAL_CONSULT_MESSAGE . '")';
			echo '</script>';
		}
	}
	*/
	...
	if($filter_option == FILTER_OPTION_PENDING) {
		if($db->hasPatientsWithConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_PENDING)) {
			$list1 = $db->getPatientsByConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_PENDING);
		}
	} else if ($filter_option == FILTER_OPTION_IN_PROGRESS) {
		if($db->hasPatientsWithConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_IN_PROGRESS)) {
			$list2 = $db->getPatientsByConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_IN_PROGRESS);
		}
	} else {
		if($db->hasPatientsWithConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_PENDING)) {
			$list1 = $db->getPatientsByConsultStatus(CONSULT_STATUS_READY_FOR_MEDICAL_CONSULT_PENDING);
...
<div id="content" class="container-fluid">
	...
	<div id="navigation_header_row" class="row">
		<div id="navigation_header_col" class="col-xs-12">
			...
			<span id="navigation_header_span"><?php echo MEDICAL_CONSULT; ?></span>
		</div>
...
<script>
	...
	function filterClick(arg) {
		window.location.href = "medical_consult_patients.php?filter_option=" + arg + getLang(1);
	}
...
function searchPatients(community_name) {
	...
	if(search_str) {
		...
		window.location.href = "search_patients.php?mode=1&search_str=" + search_str + "&consult_status=2" + lang_text;
	}
}
	...
	function patientClick(patient_id, active_consult_id) {
		...
		if(active_consult_id) {
			window.location.href = "consult_active.php?mode=3&consult_id=" + active_consult_id + lang_text;
		} else {
			window.location.href = "profile.php?mode=3&patient_id=" + patient_id + lang_text;
		}
	}
